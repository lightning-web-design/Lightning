...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_MimeEntity
{
    ...
    {
        $this->_cacheKey = md5(uniqid(microtime().mt_rand(0, 32000), true));
        $this->_cache = $cache;
    ...
    public function getBoundary()
    {
        if (!isset($this->_boundary)) {
            $this->_boundary = '_=_swift_v4_' . time() . '_' . md5(microtime().mt_rand(0, 32000).uniqid('', true)) . '_=_';
        }
    ...
    {
        $idLeft = md5(microtime().mt_rand(0, 32000) . '.' . time() . '.' . uniqid(mt_rand(), true));
